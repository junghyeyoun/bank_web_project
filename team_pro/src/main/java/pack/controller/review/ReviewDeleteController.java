...
public class ReviewDeleteController {
	 private Logger logger = LoggerFactory.getLogger(this.getClass());
	    ...
	    public String delete(@RequestParam("review_id") int review_id, @RequestParam("page") String page, RedirectAttributes redirectAttributes) {
	        try {
	            if (!confirmDeletion()) {
	                // 사용자가 삭제를 취소한 경우
	            ...
	            boolean result = reviewDao.delete(review_id);
	            if (result) {
	                return "redirect:/productlist?page=" + page;
	            } else {
